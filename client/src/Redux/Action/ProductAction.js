...
export const listProduct = (keyword="",pageNumber="") => async(dispatch) =>{
    try {
        ...
        const {data} = await axios.get(`https://crudbackend-up6i.onrender.com/api/products?keyword=${keyword}&pageNumber=${pageNumber}`);
...
export const listProductDetails = (id) => async(dispatch) =>{
    try {
        ...
        const {data} = await axios.get(` https://crudbackend-up6i.onrender.com/api/products/${id}`);
 ...
 //update PRODUCT
   export const editProduct = (product) => async (dispatch, getState) => {
    try {
      ...
      const {data} = await axios.put(`https://crudbackend-up6i.onrender.com/api/products/${product._id}`,product, config);
